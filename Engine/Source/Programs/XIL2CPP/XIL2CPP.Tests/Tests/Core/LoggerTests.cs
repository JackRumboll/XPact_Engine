...
/// <summary>
/// Tests for <see cref="Logger"/>. Per <c>/Documents/XIL2CPP.html</c> Rev 4
/// Section 12: MSBuild text format on stderr, optional streaming JSON
/// channel, thread-safe counters.
/// </summary>
/// <remarks>
/// <see cref="Logger"/> is process-global, so tests share state. The xUnit
/// collection here serialises tests within the class so the JSON channel +
/// stderr override don't race against parallel tests.
/// </remarks>
...
public sealed class LoggerTests : IDisposable
{
    ...
    public void Info_WritesMsBuildLineToStderr_AndDoesNotIncrementCounters()
    {
        ...
        Assert.Contains("info XIL2CPP000: hello, world", sw.ToString());
    ...
    public void Warning_IncrementsWarningCounter()
    {
        ...
        Assert.Equal(0, Logger.ErrorCount);
        Assert.Contains("warning XIL2CPP000: warning text", sw.ToString());
    }
    ...
    public void Error_IncrementsErrorCounter()
    {
        ...
        Assert.Contains("error XIL2CPP000: oh no", sw.ToString());
    }
    ...
    public void EmitDiagnostic_WithFile_FormatsAsMsBuildLine()
    {
        ...
        Logger.EmitDiagnostic(new DiagnosticRecord(
            ...
            "Module declares LangVersion higher than C# 12.",
            File: "Foo.cs",
            Line: 1,
            Column: 1));

        Assert.Equal(
            "Foo.cs(1,1): error XIL2CPP020: Module declares LangVersion higher than C# 12." + Environment.NewLine,
            sw.ToString());
    }
    ...
    public void ConfigureJsonChannel_TurnsChannelOn_AndEmitWritesBareJson()
    {
        ...
        Logger.EmitDiagnostic(new DiagnosticRecord(
            DiagnosticSeverity.Error,
            DiagnosticCodes.InternalCompilerError,
            "internal failure",
            File: "Foo.cs",
            Line: 42,
            Column: 13));
        ...
        Assert.Equal("XIL2CPP900", doc.RootElement.GetProperty("code").GetString());
    ...
    {
        // The actual result depends on Console.IsOutputRedirected, which is
        // environment-dependent. We just verify the mode is NOT Fd when no
        // positive FD is supplied.
    ...
    public void EmitDiagnostic_JsonChannelFailure_DoesNotBlockStderr()
    {
        ...
        Assert.Contains("error XIL2CPP000: payload", stderr.ToString());
    }
