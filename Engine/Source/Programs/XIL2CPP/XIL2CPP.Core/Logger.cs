...
/// <summary>
/// Process-global structured logger for XIL2CPP. Mirrors the XHT / XBT
/// logger pattern (dual-channel emit to stderr in MSBuild diagnostic
/// format + optional streaming JSON channel) per
/// <c>/Documents/XIL2CPP.html</c> Rev 4 Section 12.
/// </summary>
/// <remarks>
/// <para>
/// <b>MSBuild text format.</b> Every emitted record is formatted as
...
/// </para>
/// <para>
/// <b>JSON channel activation.</b> The channel is opt-in:
/// <list type="bullet">
///   <item><description>
///     Linux / Android: explicit opt-in via <c>-JsonFd=N</c> CLI flag at
///     <c>XIL2CPP.Entry</c> startup; default off.
///   </description></item>
///   <item><description>
///     Windows: auto-on when stdout is not a TTY
///     (<c>!Console.IsOutputRedirected</c>); records are stdout lines
///     prefixed with the sentinel <c>@@XIL2CPP-JSON@@ </c> so the JSON
///     shares the stdout stream with progress text without colliding.
///   </description></item>
/// </list>
/// </para>
/// <para>
/// <b>Counters.</b> <see cref="WarningCount"/> and <see cref="ErrorCount"/>
/// are thread-safe via <see cref="Interlocked"/>; XIL2CPP.Entry consults
/// them at session exit to compute the exit code.
/// </para>
/// <para>
...
public static class Logger
{
    ...
    /// <c>/Documents/XIL2CPP.html</c> Rev 4 Section 12. Includes the
    /// trailing space that delimits the sentinel from the JSON payload.
    /// </summary>
    public const string SentinelPrefix = "@@XIL2CPP-JSON@@ ";
    ...
    /// <summary>
    /// Emit an info-severity diagnostic with no source-anchor. Composes
    /// the message via
    ...
    public static void EmitDiagnostic(DiagnosticRecord record)
    {
        ...
        lock (s_writeGate)
        {
            ...
            if (s_jsonWriter is { } w)
            {
                ...
                {
                    // Best-effort: a JSON channel write failure must never
                    // block the stderr emit (already done above) or fail
                    // the build. Swallow and continue.
                }
            }
        }
    }
    ...
    // ---------------------------------------------------------------------
    // JSON channel configuration. Called by XIL2CPP.Entry at startup.
    // ---------------------------------------------------------------------

    /// <summary>
    /// Install a <see cref="StreamWriter"/> as the streaming JSON channel.
    /// Records emit as single-line JSON. When <paramref name="useSentinel"/>
    /// is true, every line is prefixed with <see cref="SentinelPrefix"/>
    /// (Windows / Linux-fallback mode); when false, lines are bare JSON
    /// (FD-based mode).
    /// </summary>
    /// <param name="writer">
    ...
    /// paths where the JSON shares the stdout stream with progress text.
    /// </param>
    ...
    // ---------------------------------------------------------------------
    // Test hooks. Exposed to XIL2CPP.Tests via InternalsVisibleTo.
    // ---------------------------------------------------------------------

    /// <summary>
    /// Redirect stderr emit to a caller-supplied writer. Used by tests to
    /// capture diagnostics without globally rebinding
    ...
    /// <summary>
    /// Auto-detect the JSON channel activation mode. Returns the mode flag
    /// the caller (XIL2CPP.Entry) should apply when wiring the channel.
    /// </summary>
    /// <param name="jsonFd">
    ...
    /// for explicit FD activation on POSIX;
    /// <see cref="JsonChannelMode.StdoutSentinel"/> for Windows /
    /// Linux-fallback when stdout is redirected.
    /// </returns>
...
/// <summary>
/// JSON channel activation mode per <c>/Documents/XIL2CPP.html</c> Rev 4
/// Section 12. Returned by <see cref="Logger.DecideJsonChannelMode(int)"/>.
/// </summary>
