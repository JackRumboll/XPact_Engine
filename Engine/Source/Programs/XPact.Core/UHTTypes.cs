...
//
// Ported from EpicGames.Core.UHTTypes. The Write(IMemoryWriter) overload on
// UHTTargetSettings is omitted because IMemoryWriter is not in the Task 0.1
// port set (we only need the BinaryArchiveWriter path for XBT/XHT bring-up).
...
namespace XPact.Core
{
	...
	{
		/// <summary>Version not set or the default is to be used.</summary>
		None,
		/// <summary></summary>
		V1,
		/// <summary></summary>
	...
	{
		/// <summary>Created by Epic and only public definitions will be visible to public users</summary>
		PublicAPI,
		/// <summary>Created by Epic and is entirely hidden from public users</summary>
		InternalAPI,
		/// <summary>Created by a public user</summary>
		PublicUser,
		/// <summary>Created by an Epic internal user</summary>
		InternalUser
	}
	...
	{
		/// <summary>No flags, should not be configured as a desired output type.</summary>
		None = 0x0,
		/// <summary>Generate params structs which will be used to initialize UObjects on the heap at runtime.</summary>
		Params = 0x1,
		/// <summary>Generate declarations of constinit variables of UObject-derived types allowing these objects to be stored in the binary's data segment.</summary>
		ConstInit = 0x2,
		/// <summary>Default setting to use</summary>
		Default = Params,
		/// <summary>Generate all possible types at once for comparison</summary>
	...
	public record struct UHTTargetSettings
	{
		...
		public UHTTargetSettings(BinaryArchiveReader reader) : this()
		{
			...
			GenerateEnumMaxValues = reader.ReadBool();
			ValidateModuleDependencies = reader.ReadBool();
		}

		public readonly void Write(BinaryArchiveWriter writer)
		{
			writer.WriteString(CompiledInObjectFormat.ToString());
			writer.WriteBool(GenerateEnumMaxValues);
			writer.WriteBool(ValidateModuleDependencies);
		}
		...
		/// <summary>
		/// Placeholder value to control whether or not UHT validates module dependencies from UBT to find missing
		/// dependencies which are needed because of generated code (e.g. FStructBaseChain references)
		/// </summary>
		public bool ValidateModuleDependencies { get; set; } = false;

		/// <summary>If true, enable the input cache read</summary>
		public bool EnableInputCacheRead { get; set; } = false;

		/// <summary>If true, enable the input cache write</summary>
		public bool EnableInputCacheWrite { get; set; } = false;
	}
	...
	public class UHTManifest
	{
		public class Module
		{
			...
			// Other modules in the manifest which this module has a link dependency on in UBT
			// Can be used to validate dependencies which are required because of generated code
			...
			public bool TryGetDefine(string name, out string? value)
			{
				...
				{
					if (!define.StartsWith(name, StringComparison.Ordinal))
					{
						continue;
					}
			...
			public bool TryGetDefine(string name, out int value)
			{
				...
				{
					return Int32.TryParse(stringValue, System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out value);
				}
				value = 0;
				return false;
			}
		...
		/// <summary>True if the current target is a game target</summary>
		public bool IsGameTarget { get; set; } = false;

		/// <summary>The engine path on the local machine</summary>
		public string RootLocalPath { get; set; } = String.Empty;

		/// <summary>Name of the target currently being compiled</summary>
		public string TargetName { get; set; } = String.Empty;

		/// <summary>File to contain additional dependencies that the generated code depends on</summary>
		public string ExternalDependenciesFile { get; set; } = String.Empty;

		/// <summary>Target-wide settings for UHT/code generation.</summary>
