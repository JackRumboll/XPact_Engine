...
namespace XPact.Core.IO
{
	...
	public class DirectoryReference : FileSystemReference, IEquatable<DirectoryReference>, IComparable<DirectoryReference>
	{
		...
		public DirectoryReference? ParentDirectory
		{
			get
			{
				...
				return new(FullName.Substring(0, parentLength), Sanitize.None);
			}
		}
		...
		public static DirectoryReference GetTempPath() => new(Path.GetTempPath());
		...
		public static DirectoryReference? GetSpecialFolder(Environment.SpecialFolder folder)
		{
			...
			return String.IsNullOrEmpty(folderPath) ? null : new(folderPath);
		}
		...
		public static DirectoryReference Combine(DirectoryReference baseDirectory, params string[] fragments) => fragments.Length == 0 ? baseDirectory : new(CombineStrings(baseDirectory, fragments), Sanitize.None);
		...
		public static void Move(DirectoryReference sourceDir, DirectoryReference destinationDir)
			=> Directory.Move(sourceDir.FullName, destinationDir.FullName);
		...
		/// <param name="sourceDir">The source directory</param>
		/// <param name="destinationDir">The destination directory</param>
		public static void Copy(DirectoryReference sourceDir, DirectoryReference destinationDir)
		{
			CreateDirectory(destinationDir);

			foreach (string file in Directory.GetFiles(sourceDir.FullName))
			{
				File.Copy(file, Path.Combine(destinationDir.FullName, Path.GetFileName(file)), true);
			}

			foreach (string directory in Directory.GetDirectories(sourceDir.FullName))
			{
				Copy(new(directory), Combine(destinationDir, Path.GetFileName(directory)));
			}
		}
		...
		public static DirectoryReference MakeRemote(string absolutePath) => new(absolutePath, Sanitize.None);
		...
		/// <summary>
		/// Create a DirectoryReference from a string. If the string is null or whitespace, returns a null DirectoryReference.
		/// </summary>
		...
		public static DirectoryReference? FromString(string? directoryName) => String.IsNullOrWhiteSpace(directoryName) ? null : new(directoryName);
		...
		public static DirectoryReference FindCorrectCase(DirectoryReference location) => new(DirectoryUtils.FindCorrectCase(location.ToDirectoryInfo()));

		/// <summary>
		/// Constructs a DirectoryInfo object from this reference
		/// </summary>
		/// <returns>New DirectoryInfo object</returns>
		public DirectoryInfo ToDirectoryInfo() => new(FullName);
		...
		public static DirectoryReference GetCurrentDirectory() => new(Directory.GetCurrentDirectory());
		...
		public static void SetCurrentDirectory(DirectoryReference location)
		{
			...
			if (XPact.Core.RuntimePlatform.Current == XPact.Core.RuntimePlatform.Type.Windows && Directory.GetCurrentDirectory().Equals(location.FullName, StringComparison.OrdinalIgnoreCase))
			{
	...
	internal class DirectoryReferenceJsonConverter : JsonConverter<DirectoryReference>
	{
		public override DirectoryReference? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			...
			return fullName is null ? null : new(fullName);
		}

		public override DirectoryReference ReadAsPropertyName(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			if (reader.TokenType != JsonTokenType.PropertyName)
			{
				throw new JsonException("DirectoryReference keys must be represented as JSON strings");
			}
		...
		public override void WriteAsPropertyName(Utf8JsonWriter writer, [DisallowNull] DirectoryReference value, JsonSerializerOptions options)
		{
	...
	public static class DirectoryReferenceExtensionMethods
	{
		...
		public static DirectoryReference? ReadDirectoryReference(this BinaryReader reader)
		{
			...
			return (fullName.Length == 0) ? null : new(fullName, DirectoryReference.Sanitize.None);
		}
		...
		{
			string? fullName = reader.ReadString();
			return fullName == null ? null : new(fullName, DirectoryReference.Sanitize.None);
		}
