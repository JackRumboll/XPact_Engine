...
//
// Ported verbatim from EpicGames.Core.BinaryArchiveWriter except for
// WriteLogEvent, which references LogEvent — not in the Task 0.1 port set.
...
namespace XPact.Core.IO
{
	...
	public sealed class BinaryArchiveWriter : IDisposable
	{
		...
		/// </summary>
		static readonly ReferenceComparer s_referenceComparerInstance = new();
		...
		/// </summary>
		readonly Dictionary<object, int> _objectToUniqueId = new(s_referenceComparerInstance);
		...
		private void WriteBulkData(Array data, int size)
		{
			if (size > 0)
			{
				for (int pos = 0; ;)
				{
					...
					Buffer.BlockCopy(data, pos, _buffer, _bufferPos, copySize);
					_bufferPos += copySize;
					pos += copySize;
