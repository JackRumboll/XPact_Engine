...
public:
	...
	void Reserve(size_t Count)
	{
		...
		for (autortfm_open_to_closed_mapping *OldEntry = Map->Entries, *FinalEntry = Map->Entries + Map->IndexMask;
			 OldEntry <= FinalEntry; ++OldEntry)
		{
			if (!OldEntry->Open)
			{
				continue;
	...
	void DumpStats()
	{
		...
		AUTORTFM_LOG("==================");
		AUTORTFM_LOG("Occupancy: %zu entries in %zu slots (load factor %g%%)", Map->EntryCount, MapSlots,
		...
		for (const autortfm_open_to_closed_mapping *Entry = Map->Entries, *FinalEntry = Map->Entries + Map->IndexMask;
			 Entry <= FinalEntry; ++Entry)
		{
			if (Entry->Open)
			{
				...
				uintptr_t Delta = (IdealIndex <= ActualIndex) ? ActualIndex - IdealIndex : ActualIndex + MapSlots - IdealIndex;
...
AUTORTFM_INTERNAL static void* FunctionMapReportError(void* Open, const char* Where)
{
#if AUTORTFM_PLATFORM_WINDOWS
	if (Open == &InitializeCriticalSection || Open == &InitializeCriticalSectionAndSpinCount || Open == &SetCriticalSectionSpinCount
		|| Open == &TryEnterCriticalSection || Open == &EnterCriticalSection || Open == &LeaveCriticalSection
		|| Open == &DeleteCriticalSection)
	{
		ForTheRuntime::ReportAutoRTFMHazard(ForTheRuntime::EHazardType::CriticalSection);
		return nullptr;
	}
	else if (Open == &TryAcquireSRWLockExclusive || Open == &AcquireSRWLockExclusive || Open == &ReleaseSRWLockExclusive
			 || Open == &TryAcquireSRWLockShared || Open == &AcquireSRWLockShared || Open == &ReleaseSRWLockShared
			 || Open == &TryAcquireSRWLockExclusive)
	{
		ForTheRuntime::ReportAutoRTFMHazard(ForTheRuntime::EHazardType::RWLock);
		return nullptr;
	}
#elif AUTORTFM_PLATFORM_LINUX
	if (Open == &pthread_mutexattr_init || Open == &pthread_mutexattr_settype || Open == &pthread_mutex_init
		|| Open == &pthread_mutexattr_destroy || Open == &pthread_mutex_destroy || Open == &pthread_mutex_trylock
		|| Open == &pthread_mutex_lock || Open == &pthread_mutex_unlock)
	{
		ForTheRuntime::ReportAutoRTFMHazard(ForTheRuntime::EHazardType::CriticalSection);
		return nullptr;
	}
	else if (Open == &pthread_rwlock_init || Open == &pthread_rwlock_destroy || Open == &pthread_rwlock_trywrlock
			 || Open == &pthread_rwlock_wrlock || Open == &pthread_rwlock_unlock || Open == &pthread_rwlock_tryrdlock
			 || Open == &pthread_rwlock_rdlock)
	{
		ForTheRuntime::ReportAutoRTFMHazard(ForTheRuntime::EHazardType::RWLock);
		return nullptr;
	}
	...
	// This is gross - but if we are allowed to recover from missing functions (the eventual mode
	// we intend to ship AutoRTFM in to users), then we will do an abort in the report error calls
	// below. Which would mean we wouldn't free the allocation backing the string below! Get around
	// it by putting it in an explicit heap allocation, and (ab)use our existing on-abort handling
	// system to free the memory.
	std::string* const FunctionDescription = new std::string(GetFunctionDescription(Open));

	FContext::Get()->GetCurrentTransaction()->DeferUntilAbort([FunctionDescription] { delete FunctionDescription; });
