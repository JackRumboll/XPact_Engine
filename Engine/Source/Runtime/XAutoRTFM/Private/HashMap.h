...
class AUTORTFM_DISABLE THashMap
{
	using FInnerMap =
		std::unordered_map<KeyType, ValueType, Hash, Equal, StlAllocator<std::pair<const KeyType, ValueType>, Allocator>>;
	...
	class TIterator
	{
		...
		using FKeyAndValue =
			std::conditional_t<bConst, const TKeyAndValue<const KeyType, ValueType>, TKeyAndValue<const KeyType, ValueType>>;
	...
	public:
		...
		FKeyAndValue& operator*() const
		{
			...
			using FPair = std::conditional_t<bConst, const std::pair<const KeyType, ValueType>, std::pair<const KeyType, ValueType>>;
