...
void DefaultLog(const char* File, int Line, [[maybe_unused]] void* ProgramCounter, autortfm_log_severity Severity,
	const char* Format, va_list Args)
{
	static constexpr int InlineBufferLength = 256;
	char InlineBuffer[InlineBufferLength];

	va_list Args2;
	va_copy(Args2, Args);
	int Count = vsnprintf(InlineBuffer, InlineBufferLength, Format, Args);
	auto AllocatedBuffer = (Count >= InlineBufferLength) ? std::unique_ptr<char[]>{new char[Count + 1]} : nullptr;
	if (AllocatedBuffer)
	{
...
autortfm_extern_api GExternAPI = {
	...
	/* Free */ nullptr,
	/* Log */ DefaultLog,
	/* LogWithCallstack */ nullptr,
	/* EnsureFailure */ nullptr,
	/* IsLogActive */ nullptr,
	/* CaptureCallstack */ nullptr,
	/* LogCallstack */ nullptr,
	/* OnRuntimeEnabledChanged */ nullptr,
	/* OnRetryTransactionsChanged */ nullptr,
#if AUTORTFM_SANITIZER
	/* OnSanitizerModeChanged */ nullptr,
	/* OnSanitizerRecordClosedWriteCallstacksChanged */ nullptr,
#endif
	/* RollbackWrite */ nullptr,
};
