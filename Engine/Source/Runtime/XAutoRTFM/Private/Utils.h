...
// trampolines, and the function is always uninstrumented.
// Note: This annotation will prevent inlining of annotated functions.
#define AUTORTFM_INTERNAL [[clang::autortfm(autortfm_mode_internal)]]
...
AUTORTFM_DISABLE __attribute__((__format__(__printf__, 4, 5))) UE_AUTORTFM_API void ReportError(
	const char* File, int Line, void* ProgramCounter, const char* Format, ...);

[[noreturn]] inline void InternalUnreachable();
...
[[clang::preserve_most, noreturn]] UE_AUTORTFM_API UE_AUTORTFM_FORCENOINLINE void DoAssert(void (*Logger)());

// We use a special calling convention for this call to minimize
// the cost of calling this function (which is super unlikely to
// be called because it is the expecting true codepath, EG. only
// things we don't expect to happen get here).
[[clang::preserve_most]] UE_AUTORTFM_FORCENOINLINE void DoExpect(void (*Logger)());
...
__attribute__((__format__(__printf__, 5, 6))) inline void Log(
	const char* File, int Line, void* ProgramCounter, autortfm_log_severity Severity, const char* Format, ...)
{
	if (ProgramCounter == nullptr)
	{
		ProgramCounter = __builtin_return_address(0);
...
__attribute__((__format__(__printf__, 2, 3))) inline void LogWithCallstack(autortfm_log_severity Severity, const char* Format, ...)
{
	va_list Args;
	va_start(Args, Format);
	GExternAPI.LogWithCallstack(__builtin_return_address(0), Severity, Format, Args);
...
// If ProgramCounter is null, then the return address of the call will be used.
__attribute__((__format__(__printf__, 5, 6))) inline void EnsureFailure(
	const char* File, int Line, void* ProgramCounter, const char* Condition, const char* Format, ...)
{
	if (ProgramCounter == nullptr)
	{
		ProgramCounter = __builtin_return_address(0);
...
inline constexpr T Log2(T Value)
{
	...
	return Value > 0 ? NumBits - __builtin_clzg(Value) - 1 : 0;
}

// RAII helper to assign a value to a variable for the lifetime of the
// FScopedAssignment, restoring the original value when the FScopedAssignment is
// destructed.
template <typename T>
struct TScopedAssignment
{
	inline TScopedAssignment(T& Variable, T NewValue) : Variable(Variable), OldValue(Variable)
	{
		Variable = NewValue;
	}

	inline ~TScopedAssignment()
	{
		Variable = OldValue;
	}

private:
	TScopedAssignment(const TScopedAssignment&) = delete;
	TScopedAssignment(TScopedAssignment&&) = delete;
	TScopedAssignment& operator=(const TScopedAssignment&) = delete;
	TScopedAssignment& operator=(TScopedAssignment&&) = delete;
	T& Variable;
	T OldValue;
};

// Assigns Value to Variable, and restores the original value of Variable at the
// end of the current scope.
#define AUTORTFM_SCOPED_ASSIGNMENT(Variable, Value) \
	TScopedAssignment UE_AUTORTFM_CONCAT(ScopedAssignment, __COUNTER__){Variable, Value};

}  // namespace AutoRTFM

#define AUTORTFM_REQUIRE_SEMICOLON static_assert(true)

#define AUTORTFM_REPORT_ERROR(Format, ...) \
	::AutoRTFM::ReportError(__FILE__, __LINE__, /* ProgramCounter */ nullptr, Format, ##__VA_ARGS__)

#define AUTORTFM_LOG_AT_SEVERITY(Severity, Format, ...) \
	::AutoRTFM::Log(__FILE__, __LINE__, /* ProgramCounter */ nullptr, Severity, Format, ##__VA_ARGS__)

#define AUTORTFM_VERBOSE(Format, ...) AUTORTFM_LOG_AT_SEVERITY(autortfm_log_verbose, Format, ##__VA_ARGS__)
#define AUTORTFM_LOG(Format, ...) AUTORTFM_LOG_AT_SEVERITY(autortfm_log_info, Format, ##__VA_ARGS__)
#define AUTORTFM_WARN(Format, ...) AUTORTFM_LOG_AT_SEVERITY(autortfm_log_warn, Format, ##__VA_ARGS__)
#define AUTORTFM_ERROR(Format, ...) AUTORTFM_LOG_AT_SEVERITY(autortfm_log_error, Format, ##__VA_ARGS__)
#define AUTORTFM_FATAL(Format, ...) AUTORTFM_LOG_AT_SEVERITY(autortfm_log_fatal, Format, ##__VA_ARGS__)

#define AUTORTFM_VERBOSE_V(Format, Args) \
	::AutoRTFM::LogV(__FILE__, __LINE__, /* ProgramCounter */ nullptr, autortfm_log_verbose, Format, Args)
#define AUTORTFM_LOG_V(Format, Args) \
	::AutoRTFM::LogV(__FILE__, __LINE__, /* ProgramCounter */ nullptr, autortfm_log_info, Format, Args)
#define AUTORTFM_WARN_V(Format, Args) \
	::AutoRTFM::LogV(__FILE__, __LINE__, /* ProgramCounter */ nullptr, autortfm_log_warn, Format, Args)
#define AUTORTFM_ERROR_V(Format, Args) \
	::AutoRTFM::LogV(__FILE__, __LINE__, /* ProgramCounter */ nullptr, autortfm_log_error, Format, Args)
#define AUTORTFM_FATAL_V(Format, Args) \
	::AutoRTFM::LogV(__FILE__, __LINE__, /* ProgramCounter */ nullptr, autortfm_log_fatal, Format, Args)

#define AUTORTFM_VERBOSE_IF(Condition, Format, ...)  \
	do                                               \
	{                                                \
		if (Condition)                               \
		{                                            \
			AUTORTFM_VERBOSE(Format, ##__VA_ARGS__); \
		}                                            \
	} while (false)
#define AUTORTFM_LOG_IF(Condition, Format, ...)  \
	do                                           \
	{                                            \
		if (Condition)                           \
		{                                        \
			AUTORTFM_LOG(Format, ##__VA_ARGS__); \
		}                                        \
	} while (false)
#define AUTORTFM_WARN_IF(Condition, Format, ...)  \
	do                                            \
	{                                             \
		if (Condition)                            \
		{                                         \
			AUTORTFM_WARN(Format, ##__VA_ARGS__); \
		}                                         \
	} while (false)
#define AUTORTFM_ERROR_IF(Condition, Format, ...)  \
	do                                             \
	{                                              \
		if (AUTORTFM_UNLIKELY(Condition))          \
		{                                          \
			AUTORTFM_ERROR(Format, ##__VA_ARGS__); \
		}                                          \
	} while (false)
#define AUTORTFM_FATAL_IF(Condition, Format, ...)  \
	do                                             \
	{                                              \
		if (AUTORTFM_UNLIKELY(Condition))          \
		{                                          \
			AUTORTFM_FATAL(Format, ##__VA_ARGS__); \
		}                                          \
	} while (false)
...
#define AUTORTFM_ENSURE(Condition)                                                                                \
	do                                                                                                            \
	{                                                                                                             \
		if (AUTORTFM_UNLIKELY(!(Condition)))                                                                      \
		{                                                                                                         \
...
#define AUTORTFM_ENSURE_MSG(Condition, Format, ...)                                                                             \
	do                                                                                                                          \
	{                                                                                                                           \
		if (AUTORTFM_UNLIKELY(!(Condition)))                                                                                    \
		{                                                                                                                       \
...
#define AUTORTFM_ENSURE_MSG_V(Condition, Format, Args)                                                                  \
	do                                                                                                                  \
	{                                                                                                                   \
		if (AUTORTFM_UNLIKELY(!(Condition)))                                                                            \
		{                                                                                                               \
...
[[noreturn]] inline void AutoRTFM::InternalUnreachable()
{
